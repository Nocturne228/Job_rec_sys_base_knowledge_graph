...
def re_basedata(basic_data, data):
    """
    使用正则表达式从文本数据中提取基础信息，包括电话、邮箱、年龄和学历。
    :param basic_data: dict, 存储基础信息的字典
    :param data: str, 待分析的文本数据
    :return: dict, 更新后的基础信息字典
    """
    if basic_data['tel'] == '':
...
def handle_basedata(data, basic_data, total_data, tokenizer3, model3):
    """
    处理基础数据，包括姓名、学校、地点的NER提取和正则匹配更新
    :param data: list, 包含文本数据的列表
    :param basic_data: dict, 存储基础信息的字典
    :param total_data: 存储最终分析结果的字典
    :param tokenizer3: Tokenizer, 用于NER模型的分词器
    :param model3: Model, 用于NER的预训练模型
    :return:
    """
    for data in data[0]:
        # 先跑NER 选出姓名 地点 学历
...
def handle_job_obj(total_data):
    """
    处理并更新求职意向信息
    :param total_data: dict, 存储最终分析结果的字典
    :return:
    """
...
def handle_job_fit(total_data, tokenizer2, model2):
    """
    基于个人信息和工作经验，使用岗位匹配模型预测合适的工作职位
    :param total_data: dict, 存储最终分析结果的字典
    :param tokenizer2: Tokenizer, 用于岗位匹配模型的分词器
    :param model2: Model, 用于岗位匹配的预训练模型
    :return:
    """
